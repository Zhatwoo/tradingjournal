...
export default function CoachAi({ kpis, filtered, currencyFormatter }) {
  ...
  const generateAIResponse = (question, userMetrics) => {
    ...
    if (lowerQuestion.includes('strategy') || lowerQuestion.includes('approach') || lowerQuestion.includes('method') || lowerQuestion.includes('best strategy')) {
      ...
      if (strategyEntries.length > 0) {
        ...
        strategyAnalysis += `• Total P&L: ${currencyFormatter(bestStrategy[1].pnl)}\n`;
        strategyAnalysis += `• Average per trade: ${currencyFormatter(bestStrategy[1].avgPnl)}\n\n`;
        ...
        if (worstStrategy[0] !== bestStrategy[0]) {
          ...
          strategyAnalysis += `• Total P&L: ${currencyFormatter(worstStrategy[1].pnl)}\n`;
          strategyAnalysis += `• Average per trade: ${currencyFormatter(worstStrategy[1].avgPnl)}\n\n`;
        }
        ...
        if (strategyEntries.length > 2) {
          ...
          strategyEntries.forEach(([name, data], index) => {
            strategyAnalysis += `${index + 1}. **${name}**: ${currencyFormatter(data.avgPnl)} avg, ${data.winRate.toFixed(1)}% win rate\n`;
          });
          strategyAnalysis += `\n`;
        }
